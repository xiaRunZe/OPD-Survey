...
- **🎯 问题**: OPD 几乎所有实现都在**教师 logit 上加 stop-gradient**(避免教师随学生一起被更新导致不稳定)。这个操作是**经验工程技巧**,没人从数学上验证它对**各种 f-散度**(不只 KL)是否成立。一旦改成 JSD、χ2、reverse-KL,stop-gradient 是否还给出**正确的 advantage 估计**?不确定。
...
- **🔧 方法**:
  ...
  4. 框架支持任意 f-散度(KL、JSD、χ2、TV 等),用户按需选。
...
- **🔧 方法**:
  ...
  3. 散点图 → 发现**惊人的线性关系**:gap 越大、最终提升越大(且 R2 极高);
...
#### 📄 [On the Geometry of On-Policy Distillation](https://arxiv.org/abs/2606.07082) | Zhennan Shen, 2026-06-05
- **🎯 问题**: OPD 已被广泛用于提升 LLM 推理,但**训练动态理解严重落后于实践** -- 它在参数空间里到底走什么轨迹?和 SFT、RLVR 是什么关系?是"中间点"还是有自己的独立机制?问题:因为没人能说清 OPD 的 update geometry,实际调参 / 选型只能靠经验试。
- **💡 思路**: 转换提问 -- **别看 loss 表面,看参数空间里更新的几何**。用一套**参数空间诊断指标**刻画 OPD 更新轨迹,与 SFT、RLVR 系统对比。问题从"OPD 好不好"变成"OPD 的 update 在参数空间上是什么形状、占据什么子空间"。
- **🔧 方法**:
  1. 构造**参数空间诊断套件**:update sparsity(涉及多少权重)、principal-direction alignment(是否走主成分方向)、update subspace rank(更新集中在多少维);
  2. 对比三组训练:SFT、RLVR、OPD,同任务同基座;
  3. 发现:OPD 处于"**off-principal 宽松区**" -- 相比 SFT 涉及更少权重且**更强避主方向**,相比 RLVR 约束更松;
  4. 发现:**subspace locking** -- OPD 累计更新**快速进入一条窄的低维通道**(不是漫无方向扩散);
  5. **控制实验**:将训练**约束在早期形成的 update subspace** -- **OPD 性能几乎保留**,但 SFT **大幅退化** → 这条 locked subspace 对 OPD 是**功能充分的**;
  6. **打乱实验**:sparsify update tokens / 跑 off-policy rollouts **保留** rank dynamics;混入 RLVR objective **改变** rank dynamics → rank 不是"训练技巧"后果,是**目标本身的几何性质**。
- **📊 效果**: 17 页 8 图的系统诊断;首次给出"**OPD 不是 SFT/RLVR 的中间点,而是诱导独立 update geometry**"的实证。SFT vs RLVR vs OPD 三者的**静态定位 + 动态锁定**两张图被详细刻画。
- **⚠️ 局限**: 诊断在特定模型 / 任务上得出,是否跨规模 / 跨任务鲁棒未充分验证;subspace locking 的"通道"宽度是否可作为新的超参 / 监控信号未展开;打乱实验只覆盖了 token sparsify / off-policy rollout / RLVR 混合三个扰动,其他扰动(如 batch size、sequence length)的影响未系统化。
- **价值**: 为"**OPD 调参黑箱**"提供**参数空间诊断语言**。把"调 OPD"从"试学习率、试温度"提升到"**观察 update geometry、监控 subspace 变化**"。后续 OPD 理论工作(如 RKLOPD、F-divergence survey)可以**复用同一诊断套件**做横向对比。
